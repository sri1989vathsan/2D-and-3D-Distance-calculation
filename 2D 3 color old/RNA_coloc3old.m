function RNA_coloc3old(mask1, mask2, mrnafile , pixelsize, radius, dist, alchk)
% Old function to calcuate 3 color distances - stores in multiple files
...
for qu = 1:3
mrna5file = mrnas(qu,1);
mrna3file = mrnas(qu,2);
files = filenames(qu);
...
if(alchk==1)
    if (~isempty(mrna5file) &&  ~isempty(mrna3file))
         if ~isempty(img2)
             vals=[];
             [mrna5_coloc_mrna3_nuc,mrna5_mrna3_coloc_val_nuc,vals]= colocalize2_3colorold(mrna5nuc, mrna3nuc,pixelsize,radius);
             mrna5nuc(vals,:) = [];
         else
             [mrna5_coloc_mrna3_nuc] = [];
             [mrna5_mrna3_coloc_val_nuc] = [];
         end
        [mrna5_coloc_mrna3_cyt,mrna5_mrna3_coloc_val_cyt,vals]= colocalize2_3colorold(mrna5cyt, mrna3cyt,pixelsize,radius);
        mrna5cyt(vals,:) = [];
...
elseif(alchk==0)
    if (~isempty(mrna5file) &&  ~isempty(mrna3file))
        if ~isempty(img2) 
            vals=[];
            [mrna5_coloc_mrna3_nuc,mincol,vals]= colocalize2_3colorold(mrna5nuc, mrna3nuc,pixelsize,radius);
            mrna5nuc(vals,:) = [];
        else
            [mrna5_coloc_mrna3_nuc] = [];
        end
%         
        vals=[];
        [mrna5_coloc_mrna3_cyt,mincol,vals]= colocalize2_3colorold(mrna5cyt, mrna3cyt,pixelsize,radius);
        mrna5cyt(vals,:) = [];
        twospotInput(mrna5cyt, mrna3cyt, mrna5_coloc_mrna3_cyt, mrna5nuc, mrna3nuc, mrna5_coloc_mrna3_nuc, pixelsize,img2,files);
    end
...
function twospotInput(spot1cyt, spot2cyt, spot1_coloc_spot2_cyt,spot1nuc, spot2nuc, spot1_coloc_spot2_nuc, pixelsize,img2,files)
...
mrna5_3Data1= zeros(size(spot1_w_spot2_cyt,1),4);
a=size(spot1_w_spot2_cyt,1);
if(a>0)
    mrna5_3Data1(:,1) = 1:1:size(spot1_w_spot2_cyt,1);
    mrna5_3Data1(:,2) = spot1_w_spot2_cyt(:,4);
    mrna5_3Data1(:,3) = spot2cyt(spot1_w_spot2_cyt(:,end),3);
    mrna5_3Data1(:,4) = spot1_w_spot2_cyt(:,end-1).*pixelsize;
end

if ~isempty(img2)
filesname = sprintf('Cytoplasmic Distances_%s.csv',files);
csvwrite(filesname,mrna5_3Data1);
else
...
if ~isempty(img2)
    mrna5_3Data2= zeros(size(spot1_w_spot2_nuc,1),4);
    a=size(spot1_w_spot2_nuc,1);
    if(a>0)
        mrna5_3Data2(:,1) = 1:1:size(spot1_w_spot2_nuc,1);
        mrna5_3Data2(:,2) = spot1_w_spot2_nuc(:,4);
        mrna5_3Data2(:,3) = spot2nuc(spot1_w_spot2_nuc(:,end),3);
        mrna5_3Data2(:,4) = spot1_w_spot2_nuc(:,end-1).*pixelsize;
    end
filesname2 = sprintf('Nuclear Distances_%s.csv',files);
csvwrite(filesname2,mrna5_3Data2);
