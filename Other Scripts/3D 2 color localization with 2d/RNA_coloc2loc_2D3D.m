function [cytval,nucval] = RNA_coloc2loc_2D3D(mask1, mask2, mrna5file, mrna3file, pixelsize, zpixelsize, radius, dist, alchk,nucval,cytval)
...
if(alchk==1)
    if (~isempty(mrna5file) &&  ~isempty(mrna3file))
         if ~isempty(img2)
             vals=[];
             [mrna5_coloc_mrna3_nuc,mrna5_mrna3_coloc_val_nuc,vals]= colocalize2loc_2d3d(mrna5nuc, mrna3nuc,pixelsize,zpixelsize,radius);
%              mrna5nuc(vals,:) = [];
         else
             [mrna5_coloc_mrna3_nuc] = [];
             [mrna5_mrna3_coloc_val_nuc] = [];
         end
        [mrna5_coloc_mrna3_cyt,mrna5_mrna3_coloc_val_cyt,vals]= colocalize2loc_2d3d(mrna5cyt, mrna3cyt,pixelsize,zpixelsize,radius);
%         mrna5cyt(vals,:) = [];
...
elseif(alchk==0)
    if (~isempty(mrna5file) &&  ~isempty(mrna3file))
        if ~isempty(img2) 
            vals=[];
            [mrna5_coloc_mrna3_nuc,mincol,vals]= colocalize2loc_2d3d(mrna5cyt, mrna3cyt,pixelsize,zpixelsize,radius);
%             mrna5nuc(vals,:) = [];
        else
            [mrna5_coloc_mrna3_nuc] = [];
        end
%         
        vals=[];
        [mrna5_coloc_mrna3_cyt,mincol,vals]= colocalize2loc_2d3d(mrna5cyt, mrna3cyt,pixelsize,zpixelsize,radius);
%         mrna5cyt(vals,:) = [];
        twospotInput(mrna5cyt, mrna3cyt, mrna5_coloc_mrna3_cyt, mrna5nuc, mrna3nuc, mrna5_coloc_mrna3_nuc, pixelsize,img2);
    end
...
csvwrite('Distances 3D.csv',mrna5_3Data1);  
csvwrite('Distances 2D.csv',mrna5_3Data3);  
