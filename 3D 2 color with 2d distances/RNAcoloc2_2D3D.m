function [cytval,nucval] = RNAcoloc2_2D3D(mask1, mask2, mrna5file, mrna3file, pixelsize, zpixelsize, radius, dist, alchk,nucval,cytval)
...
%     for m=1:length(mrna34)
%         if(round(mrna3(m,1))>=dim(1) || round(mrna3(m,1))==0)
%             mrna3(m,:) =[];
%         elseif(round(mrna3(m,2))>=dim(2) || round(mrna3(m,2))==0)
%             mrna3(m,:) =[];
%         end   
%     end
...
%     mrna54 = round(mrna5);
%     mrna5(mrna54(:,1)>=dim(1),:) = [];
%     mrna5(mrna54(:,1)==0,:) = [];
%     mrna5(mrna54(:,2)>=dim(2),:) = [];
%     mrna5(mrna54(:,2)==0,:) = [];
...
if(alchk==1)
    if (~isempty(mrna5file) &&  ~isempty(mrna3file))
         if ~isempty(img2)
             vals=[];
             [mrna5_coloc_mrna3_nuc,mrna5_mrna3_coloc_val_nuc,vals]= colocalize2_2d3d(mrna5nuc, mrna3nuc,pixelsize, zpixelsize, radius);
             mrna5nuc(vals,:) = [];
         else
             [mrna5_coloc_mrna3_nuc] = [];
             [mrna5_mrna3_coloc_val_nuc] = [];
         end
        [mrna5_coloc_mrna3_cyt,mrna5_mrna3_coloc_val_cyt,vals]= colocalize2_2d3d(mrna5cyt, mrna3cyt,pixelsize, zpixelsize, radius);
        mrna5cyt(vals,:) = [];
...
elseif(alchk==0)
    if (~isempty(mrna5file) &&  ~isempty(mrna3file))
        if ~isempty(img2) 
            vals=[];
            [mrna5_coloc_mrna3_nuc,mincol,vals]= colocalize2_2d3d(mrna5nuc, mrna3nuc,pixelsize, zpixelsize, radius);
%             mrna5nuc(vals,:) = [];
        else
            [mrna5_coloc_mrna3_nuc] = [];
        end
%         
        vals=[];
        [mrna5_coloc_mrna3_cyt,mincol,vals]= colocalize2_2d3d(mrna5cyt, mrna3cyt,pixelsize,zpixelsize, radius);
%         mrna5cyt(vals,:) = [];
        twospotInput(mrna5cyt, mrna3cyt, mrna5_coloc_mrna3_cyt, mrna5nuc, mrna3nuc, mrna5_coloc_mrna3_nuc, pixelsize,img2);
    end
...
if(a>0)
    ...
    mrna5_3Data1(:,3) = spot2cyt(spot1_coloc_spot2_cyt(:,5),4);
    mrna5_3Data1(:,4) = spot1_coloc_spot2_cyt(:,3);
end

mrna5_3Data3= zeros(size(spot1_coloc_spot2_cyt,1),4);
a=size(spot1_coloc_spot2_cyt,1);
if(a>0)
    mrna5_3Data3(:,1) = 1:1:size(spot1_coloc_spot2_cyt,1);
    mrna5_3Data3(:,2) = spot1cyt(spot1_coloc_spot2_cyt(:,1),4);
    mrna5_3Data3(:,3) = spot2cyt(spot1_coloc_spot2_cyt(:,5),4);
    mrna5_3Data3(:,4) = spot1_coloc_spot2_cyt(:,4);
end

if ~isempty(img2)
csvwrite('Cytoplasmic 3D Distances.csv',mrna5_3Data1);
csvwrite('Cytoplasmic 2D Distances.csv',mrna5_3Data3);
else
...
if ~isempty(img2)
    ...
    if(a>0)
        ...
        mrna5_3Data2(:,3) = spot2nuc(spot1_coloc_spot2_nuc(:,5),3);
        mrna5_3Data2(:,4) = spot1_coloc_spot2_nuc(:,3);
    end
    
mrna5_3Data4= zeros(size(spot1_coloc_spot2_nuc,1),4);
    a=size(spot1_coloc_spot2_nuc,1);
    if(a>0)
        mrna5_3Data4(:,1) = 1:1:size(spot1_coloc_spot2_nuc,1);
        mrna5_3Data4(:,2) = spot1nuc(spot1_coloc_spot2_nuc(:,1),3);
        mrna5_3Data4(:,3) = spot2nuc(spot1_coloc_spot2_nuc(:,5),3);
        mrna5_3Data4(:,4) = spot1_coloc_spot2_nuc(:,4);
    end
csvwrite('Nuclear 3D Distances.csv',mrna5_3Data2);
csvwrite('Nuclear 2D Distances.csv',mrna5_3Data4);
else
    mrna5_3Data2 = [];
end
