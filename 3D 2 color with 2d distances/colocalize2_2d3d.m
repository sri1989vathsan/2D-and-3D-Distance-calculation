function [out,min_coloc,vals]=colocalize2_2d3d(mrna1, mrna2, pixelsize,zpixelsize,radiuss)
...
for i=1:length(res(:,1))
    ...
    dst2= double(((rna2(:,1)-rna1(i,1))*pixelsize).^2 + ((rna2(:,2)-rna1(i,2))*pixelsize).^2);
    coloc= dst<=(radius.^2);
    summ = sum(coloc);
    [min_val,min_index] = min(dst);
    minval2 = dst2(min_index);
    res(i,1:5)=double([i, sum(coloc), sqrt(min_val), sqrt(minval2), min_index]);
