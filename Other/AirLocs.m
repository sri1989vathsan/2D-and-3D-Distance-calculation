function varargout = AirLocs(varargin)
...
%% license check
%following are the year, date, month of the start of the 31 day license
%  Y = 2013;
%  M = 06;
%  D = 01;
%  valid_license = check_license_timeout(Y,M,D);
%  if valid_license == 0
%      return
%  else
%      disp('license check OK');
%  end
...
params = set_detection_parameters(params,AIRLOCALIZE_version,varargin);
...
if strcmp(params.mode, 'single file input')
    ...
    if params.dense_spots == 0
        [detection_result,params] = perform_detection_on_single_image_once(params);
    elseif params.dense_spots ==1
        [detection_result,params] = perform_detection_on_single_image_once_dense_spots(params);
    end
