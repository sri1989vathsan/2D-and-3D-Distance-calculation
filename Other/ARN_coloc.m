...
if(~isempty(as_ernafile))
    ...
    as_erna=as_erna((as_erna(:,3)>=threshold),indexing); % select first three columns
...
header={'Nuc','s_erna-as_erna','no_s_erna', 'no_as_erna'};
...
header={'Nuc',strcat('mrna-',message),'no_mrna', strcat('no_',message)};
writeToFile(data, strcat(prefix,'spot_coloc_analysis.txt'), header);
...
write_On_image(disp_img, inten_dist, img, mrna(mrna(:,end-1)~=0,1:end), number);
header={'Nuc','s_erna-as_erna','s_erna-mrna','as_erna-mrna','as_erna-s_erna-mrna', 'no_mrna', 'no_s_erna', 'no_as_erna'};
writeToFile(data, [prefix,'spot_coloc_analysis.txt'], header);
header={'Nuc','s_erna-as_erna','s_erna-t_mrna','as_erna-t_mrna','as_erna-s_erna-t_mrna','trans_number', 'nascent_mrna','no_s_erna', 'no_as_erna', 'no_mRNAnascent_coloc_s_erna', 'no_mRNAnascent_coloc_as_erna', 'no_mRNAnascent_noColoc_erna'};
writeToFile(trans_data, [prefix,'trans_coloc_analysis.txt'], header);

header={'mRNA_Trans','nuc', 'intensity', 'no_nascent', 'is_also_Coloc with_as-erna', 'closest s-erna dist','no_s_erna', 's_erna_intensity', 's_erna_nascent'};
