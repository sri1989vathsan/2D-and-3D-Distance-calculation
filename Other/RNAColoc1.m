...
if(strcmp(as_ernafile, '3 mRNA loc file'))
elseif(strcmp(as_ernafile, ''))
    as_ernafile='';
end

if(strcmp(mrnafile, '5 mRNA loc file'))
elseif(strcmp(mrnafile, ''))
    mrnafile='';
end

if(strcmp(s_ernafile, 'mid mRNA loc file'))
elseif(strcmp(s_ernafile, ''))
    s_ernafile='';
end
...
%         RNA_coloc(mask, s_ernacorrfile, as_ernacorrfile, mrnafile, k, threshold, pix_size,intronsig, 'Corr_', label);
%         
...
%         RNA_coloc(mask, s_ernafile, as_ernacorrfile, mrnacorrfile, k, threshold, pix_size,intronsig,'Corr_', label );
%     else
...
%         RNA_coloc(mask, s_ernacorrfile, as_ernafile, mrnacorrfile, k, threshold, pix_size,intronsig,'Corr_', label);
%     end
% end

disp('**RNAColoc without correction**')
RNA_coloc(mask,s_ernafile,as_ernafile, mrnafile,k, threshold, pix_size, intronsig, '', label);
%close all;
