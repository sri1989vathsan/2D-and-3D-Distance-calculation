function RNA_coloc3kmeans(mask1, mask2, mrnafile , pixelsize, radius, dist, alchk)
...
elseif(alchk==0)
    if (~isempty(mrna5file) &&  ~isempty(mrna3file) && ~isempty(mrna3file))
        if ~isempty(img2) 
            vals=[];
            [coloc_mrna5mid3_nuc, mincol,vals]= colocalize3(mrna5nuc, mrna3nuc,mrnamidnuc,pixelsize,radius);
            mrna5nuc(vals,:) = [];
        else
            [coloc_mrna5mid3_nuc] = [];
        end
%         
        vals=[];
        [coloc_mrna5mid3_cyt, mincol,vals]= colocalize3(mrna5cyt, mrna3cyt,mrnamidcyt, pixelsize,radius);
        mrna5cyt(vals,:) = [];
        twospotInput(mrna5cyt, mrna3cyt, coloc_mrna5mid3_cyt, mrna5nuc, mrna3nuc, coloc_mrna5mid3_nuc, pixelsize,img2,files);
    end
